...
Этот набор меряет **поведение**, а не только точность. После ручной review27
старые 27 UNKNOWN перепроверены: 21 получили конкретные ожидаемые детали,
6 действительно остались «кода быть не должно». Правки хранятся явно в
``data/review27_etalon_corrections.json``. Историческая точка отсчёта боевой
системы остаётся 128/200.
