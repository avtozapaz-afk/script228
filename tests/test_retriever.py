...
def test_the_project_control_run_reproduces_exactly(retriever):
    """Контрольный прогон проекта обязан дать ровно 500 / 69 / 135.
    ...
    Числа меняются вместе с поставкой словаря и сверяются с контрольным
    прогоном автора: 541 деталь давала 485 / 70 / 149, выгрузка 01.09 —
    500 / 69 / 135. Правило то же: цифра должна совпасть до единицы, иначе
    репозиторий запускает не тот движок, что проект.
    """
    import json
    import os
    ...
    for text, expected in gold:
        ...
        if got == expected:
            ...
            wrong += 1
    assert (ok, wrong, len(gold) - ok - wrong) == (500, 69, 135)
