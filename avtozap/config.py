...
# Словарь на 581 деталь после ручного review27 — источник истины по external_code.
...
#: запроса термин обязан покрыть. Подобрано замером на живом прогоне 469:
#: 0.6 — исторически выбрано как zero-risk на старых 27 UNKNOWN до review27.
#: После ручной проверки 21 из них получили коды; порог пока сохраняется до
#: следующего живого измерения на исправленном etalon-200. Ноль отключает проверку покрытия,
#: единица требует полного совпадения.
DICTIONARY_ANSWER_MIN_COVERAGE = 0.6
...
#: Слова, которыми покупатель описывает МОТОР, а не просит деталь: «1.6 benzin»,
#: «2.0 dizel», «1.4 turbo», «2 motor». Сегментер справедливо уносит их в
#: описание машины, хотя словарь запчастей такие слова тоже знает. Нужны, чтобы
#: отличать их от настоящей детали, потерянной в описании: в живом прогоне 469
#: сегментер унёс в описание машины слово ``kompressor`` («Chevrolet Cruze 1.4
#: kompressor»), от предмета остался огрызок ``maqnit``, и заявка ушла в
#: магнитолу. Из одиннадцати таких случаев прогона настоящей потерей был ровно
#: один — остальные десять как раз эти слова, и трогать их нельзя.
ENGINE_SPEC_WORDS = {
    "benzin", "dizel", "dizeli", "dizel/benzin", "motor", "mator", "muherrik",
    "mühərrik", "turbo", "turbodizel", "hybrid", "hibrid", "elektro",
    "бензин", "дизель", "мотор", "двигатель", "турбо", "гибрид",
}

VEHICLE_BRANDS_NA = _normalized(VEHICLE_BRANDS)
ENGINE_SPEC_WORDS_NA = _normalized(ENGINE_SPEC_WORDS)
